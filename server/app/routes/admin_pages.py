...
# ---------------------------------------------------------------------------
# App-version publisher — admin sets the "latest version" the desktop app
# advertises in Settings → "בדוק עדכונים".  Self-contained inline page so it
# needs no Jinja template.
# ---------------------------------------------------------------------------
...
def app_version_save(request: Request, version: str = Form(...)):
    _require_user_or_redirect(request)
    from .. import database as _db
    ...
    return RedirectResponse(
        f"/admin/app-version?saved={quote(ver)}", status_code=303)
